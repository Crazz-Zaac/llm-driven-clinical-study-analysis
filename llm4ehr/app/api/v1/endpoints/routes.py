...
async def ingest_documents(request: IngestionRequest):
    ...
    try:
        ...
        if response.success:
            logger.info(f"Successfully ingested {response.metadata.get('ingested_count', 0)} chunks")
        else:
            logger.error(f"Ingestion failed: {response.metadata.get('error', 'Unknown error')}")
